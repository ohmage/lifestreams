...
class UserSpoutState {
	...
	public void newBatch(long batchId){
		...
		logger.info("{} Resume process for {} from {}", spout.getComponentId(), user.getUsername(), checkpoint);
	}
	private void log(){
		if(getAckedSerialId() == lastExpectedSerialId){
			if(this.failed){
				logger.info("{} Failed {} Checkpoint: {}", spout.getComponentId(), user.getUsername(), getCheckpoint());
			}else{
				logger.info("{} Finish {} Checkpoint: {}", spout.getComponentId(), user.getUsername(), getCheckpoint());
			}
		}
	}
	...
	public UserSpoutState(OhmageUser user, BaseLifestreamsSpout spout, DateTime checkpoint){
		this.user = user;
		this.spout = spout;
