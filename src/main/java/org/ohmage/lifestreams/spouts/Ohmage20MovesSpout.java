...
public class Ohmage20MovesSpout extends Ohmage20Spout<MovesSegment> {
    class MovesInfo {
    ...
    private Ohmage20Stream movesCredentialsStream;

    // record factory for MovesCredentials record in ohmage
    private StreamRecordFactory dataPointFactory = new StreamRecordFactory();

    Iterator<StreamRecord<MovesCredentialsData>> getMovesCredentialDataIterator(final Ohmage20User user, DateTime after) throws OhmageAuthenticationError, IOException {
    ...
    MovesUserCredentials getStoredMovesCredentialsFor(Ohmage20User requestee) throws OhmageAuthenticationError, IOException {
        ...
        while (iter.hasNext()) {
            ...
            MovesCredentialsData credentials = rec.getData();
            if (credentials.getUsername() != null && credentials.getUsername().equals(requestee.getId())) {
                latestRec = rec;
                break;
            }
        }
    ...
    MovesUserCredentials refreshCredentials(MovesUserCredentials oldCredentials, Ohmage20User user) throws OAuthException, OhmageAuthenticationError, IOException {
        ...
        new Ohmage20StreamClient(getRequester()).upload(movesCredentialsStream, rec.toObserverDataPoint());
    ...
    MovesInfo getMovesInfo(MovesUserCredentials credentials, Ohmage20User ohmageUser) {
        try {
            while (true) {
                try {
    ...
    List<StreamRecord<MovesSegment>> getMovesData(MovesInfo moves, Ohmage20User ohmageUser, DateTime start) {
    ...
    @Override
    protected Iterator<StreamRecord<MovesSegment>> getIteratorFor(IUser user,
                                                                  DateTime since) {
        try {
            if (!(user instanceof Ohmage20User)) {
                throw new RuntimeException("This spout is for ohmage20");
            }
            MovesUserCredentials credentials = this.getStoredMovesCredentialsFor((Ohmage20User) user);
            if (credentials != null) {
                // try to see if it is a valid credentials
                MovesInfo info = getMovesInfo(credentials, (Ohmage20User) user);
                if (info != null) {
                    List<StreamRecord<MovesSegment>> recs = getMovesData(info, (Ohmage20User) user, since);
