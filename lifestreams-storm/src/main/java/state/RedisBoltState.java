...
public class RedisBoltState  {
	...
	public RedisBoltState (TopologyContext context){
		super();
		// init properties
		this.key = context.getThisComponentId() +"-"
		...
		if(jedis.exists(key)){
			...
			for(byte[] key: prevState.keySet()){
				OhmageUser user = kryo.readObject(new Input(key), OhmageUser.class);
				UserState state = kryo.readObject(new Input(prevState.get(key)), UserState.class);
