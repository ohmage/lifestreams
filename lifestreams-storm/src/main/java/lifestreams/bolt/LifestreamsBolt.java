...
import lifestreams.utils.PendingBuffer.DataPointAndSource;
...
public abstract class LifestreamsBolt extends BaseLifestreamsBolt {
	...
	protected abstract boolean executeDataPoint(OhmageUser user,IDataPoint dp, UserState state, TimeWindow window, BasicOutputCollector collector);

	// the operation to perform upon receiving all the data points in the current time window
	protected abstract void finishWindow(OhmageUser user, UserState state, TimeWindow window, BasicOutputCollector collector);
	
	// the operation to perform when receive snapshot signal 
	abstract protected void snapshotWindow(OhmageUser user, UserState state, TimeWindow window, BasicOutputCollector collector);	
	...
	@Override
	public void execute(OhmageUser user, IDataPoint dp, UserState state, BasicOutputCollector collector, GlobalStreamId source) {
		...
		if(curTimeWindow.withinWindow(dp.getTimestamp())){
			// time window object will keep some stats for all the point received
			...
			if(pendingBuf.getPendingStreams().size() == this.inputStreams.size()){
				...
				List<DataPointAndSource> pendings = new ArrayList<DataPointAndSource>(pendingBuf.getBuffer());
				pendingBuf.clearBuffer();
				
				// update the current time window as the time of next dp
				IDataPoint nextDp = pendings.get(0).getData();
				curTimeWindow = new TimeWindow(timeWindowSize, nextDp.getTimestamp());
				state.put(CUR_TIME_WINDOW, curTimeWindow);
				
				// replay all the pending dp
				for(DataPointAndSource dataAndSource: pendings){
					execute(user, dataAndSource.getData(), state, collector, dataAndSource.getSource());
				}
			}
	...
	// the operation to perform when receive snapshot signal 
	@Override
	protected void executeCommand(OhmageUser user, Command command,UserState state, BasicOutputCollector collector){
		if(command.equals(Command.CommandType.SNAPSHOT)){
			snapshotWindow(user, state, (TimeWindow) state.get(CUR_TIME_WINDOW), collector);
		}
	}
    @Override
	...
	public LifestreamsBolt(BaseSingleFieldPeriod timeWindowSize, OhmageStream targetStream){
		super(targetStream);
		this.timeWindowSize = timeWindowSize;
	}
}
