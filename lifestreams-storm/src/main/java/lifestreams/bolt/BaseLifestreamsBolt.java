...
import lifestreams.model.LifestreamsDataPoint;
...
public abstract class BaseLifestreamsBolt extends BaseBasicBolt {
	...
	abstract protected void execute(OhmageUser user, IDataPoint dp, UserState state, BasicOutputCollector collector, GlobalStreamId source);
	// the operation to perform on each incoming data point.
	abstract protected void executeCommand(OhmageUser user, Command command,UserState state, BasicOutputCollector collector);
	...
	public void execute(Tuple input, BasicOutputCollector collector) {
		...
		if(!state.containUser(user)){
			...
			// new user method should implement the neccesary initialization for the user state 
		...
		IDataPoint dp = (IDataPoint)input.getValueByField("datapoint");
		// process data point
	...
	protected void upload(LifestreamsDataPoint dp){
		// only do upload if targetStream is specified
		if(targetStream != null){
			try {
				new OhmageStreamClient(dp.getUser()).upload(targetStream, dp.toObserverDataPoint());
	...
	private List<Integer> emitInternal(LifestreamsDataPoint dp, BasicOutputCollector collector){
		// TODO: how to deal with upload failure
		upload(dp);
		return collector.emit(new Values(dp.getUser(), dp));
	}
	
	public List<Integer> emit(LifestreamsDataPoint dp, BasicOutputCollector collector){
		dp.setSnapshot(false);
		dp.toObserverDataPoint();
		return emitInternal(dp, collector);
	}
	public List<Integer> emitSnapshot(LifestreamsDataPoint dp, BasicOutputCollector collector){
		dp.setSnapshot(true);
		return emitInternal(dp, collector);
	}
	
	private Set<String> getSubGraph(TopologyContext context, String rootId, Set<String> ret){
		ret.add(rootId);
