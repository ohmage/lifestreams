...
public class TimeLeaveReturnHome extends SimpleTask<MobilityData>{
	...
	public void finishWindow(TimeWindow window) {
		...
		if(window.getHeuristicMissingDataRate() < 0.5 
			...
			// and have samples since 8am
			
			// make a list of clusterable points
			List<ClusterableMobilityData> clusterableData = new ArrayList<ClusterableMobilityData>();
			for(int i=0; i<allPoints.size(); i++){
				clusterableData.add(new ClusterableMobilityData(i, allPoints.get(i)));
			}
			...
			List<Cluster<ClusterableMobilityData>> clusters = clusterer.cluster(clusterableData);
			
			// create the place (i.e. cluster) sequence ordered in time
			int[] placeSequence = new int[allPoints.size()];
			for(Cluster<ClusterableMobilityData> cluster: clusters){
				// assume each cluster represents a place, and assign a unique place ID to it.
				int placeID =  clusters.indexOf(cluster) + 1;
				for(ClusterableMobilityData d: cluster.getPoints()){
					// for each point in the cluster, assign the place id 
					// to the corresponding location in the placeSequence
					placeSequence[clusterableData.indexOf(d)] =  placeID;
				}
			}
			// first place in the time window
			int firstPlace = placeSequence[0];
			// the last place
			int lastPlace =  placeSequence[placeSequence.length-1];
			
			int i=0;
			if(firstPlace  != 0){
				for(i=1; i<placeSequence.length; i++){
					if(placeSequence[i] != firstPlace ){
						// find the time the user leaving from the home 
						break;
					}
				}
				// if the user ever leave the home
				if(i != placeSequence.length){
					timeLeaveHome = allPoints.get(i-1).getTimestamp();
				}
			}

			// if we the user return to the home at the end of the day
			if(firstPlace == lastPlace){
				for(i=placeSequence.length-1; i>=0; i--){
					// find the time the user return the home 
					if(placeSequence[i] != lastPlace){
						break;
					}
				}
				if(i != -1){
			...
			if(timeLeaveHome != null && timeReturnHome != null){
				...
				for(ClusterableMobilityData d: clusters.get(firstPlace-1).getPoints()){
