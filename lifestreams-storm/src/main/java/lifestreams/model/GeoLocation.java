...
public class GeoLocation {
	...
	public static class GeoLocationDeserializer extends JsonDeserializer<GeoLocation>{
		 @Override
		   public GeoLocation deserialize(JsonParser jp, DeserializationContext ctxt) throws JsonProcessingException, IOException {
			ObjectCodec oc = jp.getCodec();
			JsonNode node = oc.readTree(jp);
			
			Geo geo = new Geo(node.get("latitude").asDouble(), node.get("longitude").asDouble(), true);
			...
			if(node.get("timestamp") != null){
				...
				timestamp = new DateTime(node.get("time").asLong(), DateTimeZone.forID(node.get("timezone").asText()));
			}
	...
	public static class GeoLocationSerializer extends JsonSerializer<GeoLocation>{
		@Override
